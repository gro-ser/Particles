...
namespace Particles
{
    ...
    {
        abstract public Color GetColor(float x, float y, float Alpha);
        static float[] hsvRanges => new float[] { 0 / 6f, 1 / 6f, 2 / 6f, 3 / 6f, 4 / 6f, 5 / 6f, 6 / 6f };
        static Color[] hsvColors => new Color[] { rgb(255, 0, 0), rgb(255, 255, 0), rgb(0, 255, 0), rgb(0, 255, 255), rgb(0, 0, 255), rgb(255, 0, 255), rgb(255, 0, 0) };
...
#pragma warning restore IDE0032
        ...
        static Color argb(float a, float r, float g, float b) => Color.FromArgb((byte)(255 * a), (byte)(255 * r), (byte)(255 * g), (byte)(255 * b));

        [Serializable]
        public class Fun1 : ColorCreator
        {
            public override Color GetColor(float x, float y, float Alpha) => argb(Alpha, 1 - x, 1 - y, (x + y) / 2);
        }
        [Serializable]
        public class Fun2 : ColorCreator
        {
            public override Color GetColor(float x, float y, float Alpha) => argb(Alpha, x, y, 1 - (x + y) / 2);
        }
        [Serializable]
        public class RangeColors : ColorCreator
        {
            ...
            public override Color GetColor(float x, float y, float Alpha)
            {
                ...
                for (var i = 0; i < len - 1; ++i)
                {
                    ...
                    {
                        var l = 255 * (ranges[i + 1] - ranges[i]);
                        t -= ranges[i];
                        Color lc = colors[i], rc = colors[i + 1];
                        if (gradient) d = (float)(Math.Sin(((1 - dx) * x + (1 - dy) * y) * Math.PI));
        ...
        public class FromImage : ColorCreator
        {
            ...
            public override Color GetColor(float x, float y, float Alpha)
            {
                if (img == null || x >= 1 || y >= 1 || x < 0 || y < 0) return Color.Transparent;
